# Common Makefile for C/C++(*.cc) with TOPPERS ATK(OSEK) and JSP(É ITRON)
...
$(TOPPERS_CFG_SOURCE) $(TOPPERS_CFG_HEADER) : $(TOPPERS_JSP_CFG_SOURCE)
	...
	$(CC) -E -x c-header $(TOPPERS_JSP_CFG_SOURCE) $(addprefix -I ,$(CXX_ROOT)) $(addprefix -iquote ,$(INC_PATH)) -I. $(addprefix -I,$(TOPPERS_INC_PATH)) > tmpfile1
...
appflash:
	...
	@echo "$(WINECONSOLE) $(ROOT)/bin/$(APPFLASH) ./$(ROMBIN_TARGET)" >>./appflash.sh

.PHONY: ramboot
ramboot:
	@echo "echo Executing ramboot to upload $(RAMBIN_TARGET)..." >./ramboot.sh
	@echo "$(WINECONSOLE) $(ROOT)/bin/$(RAMBOOT) ./$(RAMBIN_TARGET)" >>./ramboot.sh
